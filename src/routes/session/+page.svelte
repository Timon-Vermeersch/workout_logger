<script lang="ts">
	...
	function goToNextDayTest() {
		...
		let destinationOfTodaysExercises = $completedProgramDaysHistory;

		// let finalObjectToPush:
		console.log('today:', todaysExercises);

		console.log('destination:', destinationOfTodaysExercises);
	}
</script>
	...
	<div class="w-96 max-w-full bg-gray-800 p-6 text-white">
		...
		<div class="max-h-64 space-y-3 overflow-y-auto">
			{#each $Exerciseslist.exercises as exercise}
				<button
					class={`block w-full rounded p-3 ${selectedSwapExercise === exercise ? 'bg-green-600' : 'bg-gray-600'} hover:bg-gray-500`}
